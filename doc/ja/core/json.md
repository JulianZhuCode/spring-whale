...
- ✅ **数値オーバーフロー保護** - Long と Integer の逆直列化時に自動 overflow チェック
- ✅ **浮動小数点精度制御** - Double と Float の自動精度保持
...
spring:
  whale:
    json:
      ...
      # Float/Double の小数点以下の桁数（デフォルト：8）
      float-precision: 8
```
...
### Long/Integer オーバーフロー保護

Long と Integer の逆直列化時、入力値が型の範囲を超えた場合、例外がスローされます。
...
// Long.MAX_VALUE を超える、例外をスロー
{"value": 1234567890123456789000000}

// Long.MIN_VALUE を超える、例外をスロー
{"value": -1234567890123456789000000}
```
...
#### 2. Integer のオーバーフロー保護

```json
// 通常の値
{"value": 2147483647}

// Integer.MAX_VALUE を超える、例外をスロー
{"value": 3000000000}

// Integer.MIN_VALUE を超える、例外をスロー
{"value": -3000000000}
```
...
// POST /users
{
  ...
  "latitude": 39.9042,  // 設定された精度を自動的に保持
  "accuracy": 10.5  // 設定された精度を自動的に保持
}
```
...
- Long と Integer の逆直列化時には自動的にオーバーフローをチェック
- オーバーフローが発生すると `DatabindException` 例外をスロー
- フロントエンドから送信された数値パラメータを受け取る際は、適切な例外処理を行うことを推奨

### 7. 浮動小数点の精度

- Double と Float は `float-precision` 設定に応じて自動的に精度を保持
- 精度制御には BigDecimal を使用して正確な計算を行います
- JavaScript などの言語の浮動小数点の精度の問題を回避
