...
class SmartDataScopeHandlerTest {
    ...
    void testSkipDataScopeCacheHit() {
        loginAs(1L);
        when(cache.get(DataScopeCacheKey.skipDataScope(1L), Boolean.class)).thenReturn(true);
    ...
    void testSkipDataScopeCacheMiss() {
        loginAs(1L);
        when(cache.get(DataScopeCacheKey.skipDataScope(1L), Boolean.class)).thenReturn(null);
        when(remoteApi.skipDataScope(1L)).thenReturn(new DataScopeSkipResponse(true));

        boolean result = handler.skipDataScope();

        assertThat(result).isTrue();
        verify(cache).put(DataScopeCacheKey.skipDataScope(1L), true, Duration.ofMinutes(5));
        verify(cache).put(DataScopeCacheKey.fallbackSkipDataScope(1L), true, Duration.ofMinutes(30));
    }
    ...
    void testSkipDataScopeFallbackCacheHit() {
        loginAs(1L);
        when(cache.get(DataScopeCacheKey.skipDataScope(1L), Boolean.class)).thenReturn(null);
        when(remoteApi.skipDataScope(1L)).thenThrow(new RuntimeException("Network error"));
        when(cache.get(DataScopeCacheKey.fallbackSkipDataScope(1L), Boolean.class)).thenReturn(true);
    ...
    void testSkipDataScopeFallbackCacheMiss() {
        ...
        when(cache.get(DataScopeCacheKey.skipDataScope(1L), Boolean.class)).thenReturn(null);
        when(remoteApi.skipDataScope(1L)).thenThrow(new RuntimeException("Network error"));
        when(cache.get(DataScopeCacheKey.fallbackSkipDataScope(1L), Boolean.class)).thenReturn(null);
    ...
    void testSkipDataScopeNullResponse() {
        loginAs(1L);
        when(cache.get(DataScopeCacheKey.skipDataScope(1L), Boolean.class)).thenReturn(null);
        when(remoteApi.skipDataScope(1L)).thenReturn(null);
    ...
    void testSkipTenantScopeCacheHit() {
        loginAs(1L);
        when(cache.get(DataScopeCacheKey.skipTenantScope(1L), Boolean.class)).thenReturn(true);
    ...
    void testSkipTenantScopeCacheMiss() {
        loginAs(1L);
        when(cache.get(DataScopeCacheKey.skipTenantScope(1L), Boolean.class)).thenReturn(null);
        when(remoteApi.skipTenantScope(1L)).thenReturn(new DataScopeSkipResponse(true));

        boolean result = handler.skipTenantScope();

        assertThat(result).isTrue();
        verify(cache).put(DataScopeCacheKey.skipTenantScope(1L), true, Duration.ofMinutes(5));
        verify(cache).put(DataScopeCacheKey.fallbackSkipTenantScope(1L), true, Duration.ofMinutes(30));
    }
    ...
    void testSkipTenantScopeFallbackCacheHit() {
        loginAs(1L);
        when(cache.get(DataScopeCacheKey.skipTenantScope(1L), Boolean.class)).thenReturn(null);
        when(remoteApi.skipTenantScope(1L)).thenThrow(new RuntimeException("Network error"));
        when(cache.get(DataScopeCacheKey.fallbackSkipTenantScope(1L), Boolean.class)).thenReturn(true);
    ...
    void testSkipTenantScopeFallbackCacheMiss() {
        ...
        when(cache.get(DataScopeCacheKey.skipTenantScope(1L), Boolean.class)).thenReturn(null);
        when(remoteApi.skipTenantScope(1L)).thenThrow(new RuntimeException("Network error"));
        when(cache.get(DataScopeCacheKey.fallbackSkipTenantScope(1L), Boolean.class)).thenReturn(null);
    ...
    void testResolveDeptIdsCacheHit() {
        loginAs(1L);
        String cacheKey = DataScopeCacheKey.resolveDeptIds(1L, DataScopeType.DEPT, "order");
    ...
    void testResolveDeptIdsCacheMiss() {
        ...
        when(remoteApi.resolveDeptIds(1L, DataScopeType.DEPT, "order"))
        ...
        verify(cache).put(DataScopeCacheKey.fallbackResolveDeptIds(1L, DataScopeType.DEPT, "order"),
    ...
    void testResolveDeptIdsFallbackCacheHit() {
        ...
        when(cache.getList(DataScopeCacheKey.fallbackResolveDeptIds(1L, DataScopeType.DEPT, "order")))
    ...
    void testResolveDeptIdsNullResponse() {
        ...
        String cacheKey = DataScopeCacheKey.resolveDeptIds(1L, DataScopeType.DEPT, "order");
        when(cache.getList(cacheKey)).thenReturn(null);
        when(remoteApi.resolveDeptIds(1L, DataScopeType.DEPT, "order")).thenReturn(null);
    ...
    void testConstructorReadsTtlFromProperties() {
        ...
        cacheProps.setFallbackTtl(Duration.ofHours(1L));
        ...
        verify(cache).put(eq(DataScopeCacheKey.skipDataScope(1L)), eq(true), eq(Duration.ofMinutes(10)));
        verify(cache).put(eq(DataScopeCacheKey.fallbackSkipDataScope(1L)), eq(true), eq(Duration.ofHours(1L)));
    }
}